package dev.jlo.archimedes;

import dev.jlo.archimedes.bukkit.BukkitCollisionVolumeManager;
import dev.jlo.archimedes.bukkit.BukkitScannerWorld;
import dev.jlo.archimedes.bukkit.BukkitShipEntityCarrier;
import dev.jlo.archimedes.bukkit.BukkitShipRenderer;
import dev.jlo.archimedes.bukkit.BukkitShipRiderTracker;
import dev.jlo.archimedes.bukkit.BukkitWorldMutator;
import dev.jlo.archimedes.command.ShipCommand;
import dev.jlo.archimedes.command.ShipTabCompleter;
import dev.jlo.archimedes.config.ShipConfig;
import dev.jlo.archimedes.config.ShipConfigLoader;
import dev.jlo.archimedes.model.Ship;
import dev.jlo.archimedes.render.RenderSurface;
import dev.jlo.archimedes.ship.ShipRuntime;
import dev.jlo.archimedes.ship.ShipRuntimeImpl;
import dev.jlo.archimedes.ship.ShipService;
import dev.jlo.archimedes.ship.ShipServiceImpl;
import dev.jlo.archimedes.store.ShipStore;
...
public final class ArchimedesPlugin extends JavaPlugin {
  ...
  public void onEnable() {
    ...
    try {
      ...
      dev.jlo.archimedes.buoyancy.BuoyancySurface buoyancySurface =
          new dev.jlo.archimedes.bukkit.BukkitBuoyancySurface(world);
      dev.jlo.archimedes.buoyancy.BuoyancyEngine engine =
          new dev.jlo.archimedes.buoyancy.BuoyancyEngine(
              config.gravity(),
              config.waterDensity(),
              config.blockDensity(),
              config.damping(),
              config.physicsTicks());
      dev.jlo.archimedes.buoyancy.BuoyancyImpl buoyancy =
          new dev.jlo.archimedes.buoyancy.BuoyancyImpl(
  ...
  private void registerCommand(ShipConfig config) {
    ...
    ShipCommand executor =
        new ShipCommand(
            service,
            config,
            new dev.jlo.archimedes.command.BukkitTargetResolver(config.targetDistance()));
  ...
  private static final class StoreAdapter implements dev.jlo.archimedes.ship.ShipStoreLike {
    /** Persistent store. */
    ...
    public Map<UUID, Ship> loadAll() {
      try {
        ...
        throw new dev.jlo.archimedes.ship.ShipRuntimeException(failure);
      }
    }
