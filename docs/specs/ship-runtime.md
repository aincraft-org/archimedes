...
- PDC keys derive from `ShipsPlugin.shipKey()`: base `ship-id`; derived block key = namespace + key + `-block`. Keep the derivation in one place (renderer and collision manager both use it) so stale-entity sweeps stay symmetric.
