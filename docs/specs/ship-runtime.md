...
- Hull picture stays one untransformed `BlockDisplay` per non-cloth captured block (`setBlock` only). Cloth (`*_wool`, `*_banner`, `*_wall_banner`) is tessellated `BlockDisplay` plates from `SailMesh` — not one cube per cloth cell, and not a resource-pack / `ItemDisplay` model. A 1-cell-thick wall stays a thin sheet; a multi-depth region emits a plate per cell at that cell's own depth. Sampled wind billows plate origin/rotation. Other curved looks (`ItemDisplay` + pack model, or a later bone engine) remain Future overlays. See `docs/superpowers/specs/2026-08-17-curved-mesh-rendering-review.md`.
