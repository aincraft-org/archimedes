...
- Current runtime is bound to the primary Bukkit world only. Command resolution may identify another world, but assembly there is rejected; cross-world runtime support remains Future. Disabled-world policy rejects both the primary bound world and non-primary command targets.
- Ownership: ships are player-owned; disassembly requires owner or operator.
- Errors: Current command output owns operation prefixes; service and adapter failures provide reason-only messages, preventing duplicated wording. `ShipRuntimeException` represents runtime faults; plugin disables on enable/load/reconciliation failure.
...
- `docs/superpowers/specs/2026-08-13-ship-building-design.md` is stale (immutable ships, barrier decks, capability checks, inspect reports owner+origin).
  - Stale Javadocs/wording: `ShipService.removeAllRuntime` and plugin metadata have been corrected; historical design documents retain their original decisions.
